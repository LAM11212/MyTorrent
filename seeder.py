...
def handle_handshake(conn) -> bool:
    ...
    sdrprotocol = b'FemboyTorrent'
    sdrpstrlen = len(sdrprotocol)
    reserved = b'\x00' * 8

    sdrhandshake_msg = struct.pack(f'B{sdrpstrlen}s8s32s20s', sdrpstrlen, sdrprotocol, reserved, ensure_hash, peer_id)
    conn.sendall(sdrhandshake_msg)
